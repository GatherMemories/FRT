...
//            BackupFileLoader.addBackupFile(targetPath);
    ...
    public static boolean replaceFile(Path sourcePath, Path targetPath, OperationRecord record, boolean dryRun) {
        try {
            ...
            // 替换前必须先成功备份原文件——备份失败则中止替换（不覆盖目标），
            // 否则原文件被覆盖后无法恢复（恢复/回滚全部依赖备份，用户实测数据丢失风险）
            if (!BackupFileLoader.addBackupFile(targetPath)) {
                record.setSuccess(false);
                record.setErrorMessage("替换前备份原文件失败，已取消替换（目标文件未改动，可安全重试）");
                return false;
            }
    ...
    public static boolean deleteFile(Path filePath, OperationRecord record, boolean dryRun) {
        try {
            ...
            // 删除前必须先成功备份原文件——备份失败则中止删除（不删除文件），
            // 否则文件被删后无法恢复（恢复/回滚全部依赖备份，用户实测数据丢失风险）
            if (!BackupFileLoader.addBackupFile(filePath)) {
                record.setSuccess(false);
                record.setErrorMessage("删除前备份原文件失败，已取消删除（文件未改动，可安全重试）");
                return false;
            }
